...
export const patchIntroduction = async (introduction: IntroDuctionRequest) => {
  const response = await api.patch('/api/v1/users/resumes/introduction', introduction);
  return response.data;
};
...
export const getWorkExperience = async(id: number) =>{
  const response = await api.get(`/api/v1/users/resumes/work-experiences/${id}/details`);
  return response.data;
}

// 7.5 경력 생성하기
export const postWorkExperience = async (workExperience: WorkExperienctRequest) => {
  const response = await api.post(`/api/v1/users/resumes/work-experiences`, workExperience);
  return response.data;
};
...
export const patchWorkExperience = async ({
  ...
  const response = await api.patch(`/api/v1/users/resumes/work-experiences/${id}`, workExperience);
  return response.data;
};

// 7.16 (유학생) 경력 삭제하기
export const deleteWorkExperience = async (id: number) => {
  const response = await api.delete(`/api/v1/users/resumes/work-experiences/${id}`);
  return response.data;
};

// 7.3 학력 상세 조회하기
export const getEducation = async(id: number) =>{
  const response = await api.get(`/api/v1/users/resumes/educations/${id}/details`);
  return response.data;
}
...
export const postEducation = async ({
  ...
  const response = await api.post(`/api/v1/users/resumes/educations/${id}`,education);
  return response.data;
};
...
export const patchEducation = async ({
  ...
  const response = await api.patch(`/api/v1/users/resumes/educations/${id}`,education);
  return response.data;
};
...
export const patchLanguagesLevel = async ({
  ...
  const response = await api.patch(`/api/v1/users/resumes/languages/${type}`, {level: level});
  return response.data;
};

// 7.7 언어 - ETC 생성하기
export const postEtcLanguageLevel = async () => {
  const response = await api.post('/api/v1/users/resumes/languages/additional-languages');
  return response.data;
};
...
export const patchaEtcLanguageLevel = async ({
  ...
  const response = await api.patch(`/api/v1/users/resumes/languages/additional-languages/${id}`, language);
  return response.data;
};

// 7.18 (유학생) 언어 - ETC 삭제하기
export const deleteEtcLanguageLevel = async (id: number) => {
  const response = await api.delete(`/api/v1/users/resumes/languages/additional-languages/${id}`);
  return response.data;
};
...
export const getSearchSchools = async ({
  ...
  const response = await api.get(`/api/v1/users/schools/brief?search=${search}&page=${page}&size=${size}`);
  return response.data;
};