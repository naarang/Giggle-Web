...
export const getUserProfile = async() =>{
  const response = await api.get('/api/v1/users/details');
  return response.data;
}

// 3.3 (유학생) 유저 요약 정보 조회하기
export const getUserSummaries = async() =>{
  const response = await api.get('/api/v1/users/summaries');
  return response.data;
}
...
export const patchUserProfile = async ({
  ...
  const response = await api.patch('/api/v1/users', formData);
  return response.data;
}

// 3.7 (유학생/고용주) 알람 설정 변경하기
export const patchNotificationAllowed = async(is_notification_allowed: boolean) =>{
  const response = await api.patch('/api/v1/notification-allowed',
    {
...
export const patchUserLanguage = async(language: UserLanguageRequest) =>{
  const response = await api.patch('/api/v1/users/languages', language);
  return response.data;
}
...
export const getBookmarks = async({
  ...
  const response = await api.get(`/api/v1/users/book-marks/overviews?page=${page}&size=${size}`);
  return response.data;
}

// 5.2 (유학생) 북마크 현황(개수) 확인하기
export const getBookmarksCounts = async() =>{
  const response = await api.get('/api/v1/users/book-marks/counts');
  return response.data;
}

// 6.4 (유학생) 지원 현황(개수) 확인하기
export const getApplicationCounts = async() =>{
  const response = await api.get('/api/v1/users/user-owner-job-postings/counts');
  return response.data;
}
